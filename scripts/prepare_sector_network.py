...
def add_biochar(n, costs):
    ...
    # calculate biomass and biochar heat buses
    ...
    # add biochar heat buses, stores and links in case of outputting heat generated by biochar into urban central heat system is enabled
    if snakemake.config["sector"]["heating"] and snakemake.config["sector"]["biochar"]["heat_output"]:
        logger.info("Adding capability to output heat generated by biochar into urban central heat system.")
        biochar_heat_buses = []
    ...
    # add biochar transport links
    if snakemake.config["sector"]["biochar"]["transport"]:
        logger.info("Adding capability to transport biochar.")

        transport_costs = pd.read_csv(snakemake.input.biomass_transport_costs, index_col = 0)
        transport_costs = transport_costs.squeeze()
        biochar_transport = create_network_topology(n, "biochar transport ", bidirectional = False)
